...
void setLevel(bool easy) {
    if (easy) {
        ...
        PipeTexture.loadFromFile("img/pipe-red.png");
        texture.loadFromFile("img/redbirdd.png");
        speed = 5;
        Pipe_space = 300;
        gap = 170;
    }
...
private:
    ...
    const float gravity = 9.81f * 0.37795275591f * 0.166f;
    const float maxSpeed = 12;
    const float jumpHeight = 125;
    float groundHeight;
    float avDiameter;
    bool isFlying = false;
...
public:
    ...
    void draw()
    {
        ...
        if (!hasCollided && (Keyboard::isKeyPressed(sf::Keyboard::Space) || Mouse::isButtonPressed(sf::Mouse::Left)))
        {
            if (!isPressing)
            {
                WingSound.play();
                ...
                isPressing = true;
                framesLeftToFly = 5;
                movementVelocity.y = -sqrtf(2 * 9.81f * 0.03779527559f * jumpHeight);
            }
        ...
        {
            if (rect.getPosition().x - 10 >= Pipes.at(current_pipe).pos - rect.getGlobalBounds().width / 2 && rect.getPosition().x <= Pipes.at(current_pipe).pos + PipeSprite.getGlobalBounds().width)
            {
